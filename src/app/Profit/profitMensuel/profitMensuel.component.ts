...
@Component({
  selector: 'app-profitMensuel',
  templateUrl: './profitMensuel.component.html',
  styleUrls: ['./profitMensuel.component.scss']
})
export class profitMensuelComponent implements OnInit{
  ...
  getArticles()  {
    ...
    this.server.getprofitDetaille().subscribe((data) =>{
        ...
        for(let i=0; i<this.articlesAchetes.length; i++){
          if (String(this.articlesAchetes[i].date).substring(0,8)===String(this.date).substring(0,8)){
  ...
  getProfitTotal(){
    this.server.getprofitTotal().subscribe((data)=>{
        ...
        for(let i=0; i<this.totals.length; i++){
          if(String(this.totals[i].date).substring(0,8)===String(this.date).substring(0,8)){
